...
import { Database, Plus, Trash2, Clock, ChevronLeft, X } from "lucide-react";

interface WebAppSession {
  id: string;
  prompt: string;
  schemaModels: Array<{ name: string; fields: string }>;
  compiledApp: any | null;
  timestamp: string;
}

interface LeftSidebarProps {
  accentColor: string;
  history: WebAppSession[];
  activeHistoryId: string | null;
  onSelectHistory: (item: WebAppSession) => void;
  onNewSession: () => void;
  onClearHistory: () => void;
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-3">
        ...
        {(!history || history.length === 0) ? (
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center space-y-2">
            <Database className="w-8 h-8 text-zinc-700 stroke-1" />
            <p className="text-[10px] text-zinc-600 font-mono uppercase tracking-wider">No backend builds</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    <Database className="w-3.5 h-3.5 text-zinc-500 group-hover:text-zinc-300 transition-colors shrink-0" style={isActive ? { color: accentColor } : {}} />
                  ...
                  <div className="flex items-center justify-between text-[9px] font-mono text-zinc-500 mt-1">
                    <span>{(item?.schemaModels || []).length} Models</span>
