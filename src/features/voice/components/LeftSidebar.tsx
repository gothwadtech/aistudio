...
import { Mic, Plus, Trash2, Clock, Volume2, ChevronLeft, X } from "lucide-react";
...
interface LeftSidebarProps {
  ...
  onSelectHistory: (item: VoiceSession) => void;
  onNewSession: () => void;
  onClearHistory: () => void;
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-3">
        ...
        {(!history || history.length === 0) ? (
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center space-y-2">
            <Mic className="w-8 h-8 text-zinc-700 stroke-1 animate-pulse" />
            <p className="text-[10px] text-zinc-600 font-mono uppercase tracking-wider">No dialogue logs</p>
          </div>
        ) : (
          <div className="space-y-1">
            {history.map((item) => {
              const isActive = activeHistoryId === item.id;
              // Extract preview text from the first user transcript, or use default
              const userMsg = (item?.transcripts || []).find(t => t.role === "user")?.text || "Dialog session";
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    <Volume2 className="w-3.5 h-3.5 text-zinc-500 group-hover:text-zinc-300 transition-colors shrink-0" style={isActive ? { color: accentColor } : {}} />
                  ...
                  <div className="flex items-center justify-between text-[9px] font-mono text-zinc-500 mt-1">
                    <span>Model: {item.voiceModel.replace("neural_", "")}</span>
