...
import { Message } from "../common/ai/types";
...
export default function ChatStudio({ 
  ...
  const handleGroqApiKeyChange = (val: string) => {
    onSetGroqApiKey?.(val);
    safeStorage.setItem("gothwad_groq_key", val);
  };
    ...
    <div className="flex-1 flex overflow-hidden w-full h-full bg-zinc-950 text-zinc-300 font-sans relative">
      ...
      {showLeftSidebar && (
        <>
          ...
          />
          <div className="absolute left-0 top-0 h-full z-40 shrink-0 shadow-2xl bg-zinc-900 border-r border-zinc-850 animate-[slideInLeft_0.2s_ease-out]">
            <LeftSidebar
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className={`p-1.5 bg-zinc-950 hover:bg-zinc-850 border border-zinc-800 rounded-lg transition-all cursor-pointer active:scale-95 flex items-center justify-center shrink-0 ${
                showParametersPanel ? "text-white border-zinc-700 bg-zinc-900" : "text-zinc-400 hover:text-zinc-200"
              }`}
      ...
      {/* 3. Right Side Config Parameter Panel (collapsible, floating drawer with backdrop) */}
      {showParametersPanel && (
        <>
          {/* Backdrop overlay to close when clicking outside */}
          <div 
          ...
          />
          <div className="absolute right-0 top-0 h-full z-40 shrink-0 shadow-2xl bg-zinc-900 border-l border-zinc-850 w-[280px] flex flex-col overflow-hidden select-none animate-[slideInRight_0.2s_ease-out]">
