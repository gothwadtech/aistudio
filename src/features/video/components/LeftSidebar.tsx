...
import { Film, Plus, Trash2, Clock, ChevronLeft, Play, X } from "lucide-react";
...
interface LeftSidebarProps {
  accentColor: string;
  history: VideoSession[];
  activeHistoryId: string | null;
  onSelectHistory: (item: VideoSession) => void;
  onNewSession: () => void;
  onClearHistory: () => void;
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-3">
        ...
        {(!history || history.length === 0) ? (
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center space-y-2">
            <Film className="w-8 h-8 text-zinc-700 stroke-1" />
            <p className="text-[10px] text-zinc-600 font-mono uppercase tracking-wider">No clip archives</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    <Play className="w-3.5 h-3.5 text-zinc-500 group-hover:text-zinc-300 transition-colors shrink-0" style={isActive ? { color: accentColor } : {}} />
                  ...
                  <div className="flex items-center justify-between text-[9px] font-mono text-zinc-500 mt-1">
                    <span>{item.motion.replace("_", " ")}</span>
                    <span>{item.duration} ({item.fps}fps)</span>
                  </div>
                </button>
              );
            })}
