...
import { Image, Plus, Trash2, Clock, ChevronLeft, X } from "lucide-react";
...
interface LeftSidebarProps {
  accentColor: string;
  history: ImageSession[];
  activeHistoryId: string | null;
  onSelectHistory: (item: ImageSession) => void;
  onNewSession: () => void;
  onClearHistory: () => void;
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-3">
        ...
        {(!history || history.length === 0) ? (
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center space-y-2">
            <Image className="w-8 h-8 text-zinc-700 stroke-1" />
            ...
            {history.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-xl border flex gap-3 transition-all cursor-pointer group ${
                    isActive
                ...
                >
                  {item.generatedImage && (
                    <img 
                      ...
                      alt="Thumbnail" 
                      className="w-8 h-8 rounded-lg object-cover bg-zinc-950 border border-zinc-850 group-hover:scale-105 transition-transform shrink-0"
                  ...
                  <div className="min-w-0 flex-1 flex flex-col justify-center">
                    ...
                    <span className="text-[9px] font-mono text-zinc-500 mt-0.5">
                      {item.activeStyle} ({item.activeRatio})
                    </span>
                  </div>
                </button>
              );
